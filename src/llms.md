...
| `autorize list <name>`   | Dump every iteration as markdown (oldest-first, one `##` section per iteration carrying its `summary`). Plain markdown when piped/redirected (zero ANSI escapes), ANSI-colorized on a TTY. `--color <auto\|always\|never>` overrides detection. Read-only. |
