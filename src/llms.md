...
| `autorize list <name>`   | Dump every iteration as markdown (newest-first, one `##` section per iteration carrying its `summary`). Plain markdown when piped/redirected (zero ANSI escapes), ANSI-colorized on a TTY. `--color <auto\|always\|never>` overrides detection. Read-only. |
