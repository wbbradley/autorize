...
pub struct BackfillArgs {
    ...
    /// Regenerate summaries for *every* eligible record, overwriting existing
    /// ones, instead of only filling records that are missing a summary. Use
    /// this to re-summarize a whole experiment after changing
    /// `summarize.command` (`noop`/`killed` records and those whose artifacts
    /// are gone are still skipped).
    #[arg(long)]
    pub force: bool,
}
...
fn run_with_root(args: BackfillArgs, project_root: PathBuf) -> Result<()> {
    ...
    let changed = iteration::backfill_missing_summaries(&cfg, &paths, &mut records, args.force)?;
    match (changed, args.force) {
        (true, true) => println!("regenerated summaries (--force)"),
        (true, false) => println!("backfilled missing summaries"),
        (false, true) => println!("no eligible records to regenerate"),
        (false, false) => println!("no missing summaries to backfill"),
    }
    Ok(())
}
...
{summarize}"#
    ...
    fn backfill(name: &str, root: &Path) -> Result<()> {
        backfill_with_force(name, root, false)
    }

    fn backfill_with_force(name: &str, root: &Path, force: bool) -> Result<()> {
        run_with_root(
            BackfillArgs {
                name: name.to_string(),
                force,
    ...
    #[test]
    fn backfill_force_overwrites_existing_summary() {
        // A record that already carries a summary is left alone by a normal
        // backfill, but `--force` regenerates it from the iter artifacts.
        let (tmp, root) = make_exp("test");
        write_config(&root, true);

        let paths = ExperimentPaths::new(tmp.path().to_path_buf(), "test".to_string());
        seed_iter_artifacts(&paths, 1);
        let records = vec![mk_rec(
            1,
            Outcome::Merged,
            Some(0.1),
            "stale chatty summary",
        )];
        storage::rewrite_iterations(&paths.iterations_log(), &records).unwrap();

        // Without --force the preexisting summary is untouched.
        backfill_with_force("test", tmp.path(), false).unwrap();
        let on_disk = storage::read_iterations(&paths.iterations_log()).unwrap();
        assert_eq!(on_disk[0].summary, "stale chatty summary");

        // With --force it is regenerated by the stub summarizer.
        backfill_with_force("test", tmp.path(), true).unwrap();
        let on_disk = storage::read_iterations(&paths.iterations_log()).unwrap();
        assert_eq!(on_disk[0].summary, "SUMMARY_MARKER");
        assert_eq!(
            fs::read_to_string(paths.iter_dir(1).join("summary.md")).unwrap(),
