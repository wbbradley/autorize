...
> prompt recent-iterations `reason` column + `describe_failure`) is **done**; A2 and B build on it.

- **[feature] A2 — model-generated per-iteration summary (`[summarize]` + `summary`)** — Add `summary: String` with `#[serde(default)]` to `IterationRecord` (src/storage.rs:43-56) — back-compatible with existing `iterations.jsonl`. Holds a 1-2 sentence "what this iteration attempted and why it moved the score", written by a **separate summarization step that runs AFTER the worker agent is killed/exits** (decision locked): its own command + timeout so (i) it can run a cheaper/weaker model and (ii) the summary is never truncated by the worker's iteration budget. New config section:
    ```toml
    [summarize]
    enabled = true
    command = "claude --model haiku --print {prompt_file}"   # weaker model by default
    timeout = "60s"
    stdin = "none"                                           # mirror [agent]
    ```
  (Touch src/config.rs for the schema + validation, src/templates/config.toml.tmpl for the default, and the `llms_doc_mentions_every_config_field` test will enforce documenting the keys.) Implement as a new sub-step in `run_iteration` (src/iteration.rs), gated on `summarize.enabled` and on the iteration having produced a diff (skip `Noop`). The step builds a summary prompt from the iteration's own artifacts — `iter-NNNN/changes.diff`, a tail of `agent.stdout`/`agent.stderr`, and the outcome+score+best context (default: diff + stdio + outcome/score only, **not** guidance/prior summaries) — writes it to `iter-NNNN/summary-prompt.md`, runs the summarize command via the existing subprocess machinery (reuse src/agent.rs `substitute()` / src/subproc.rs; `{prompt_file}`/`{workdir}`/`{iter}` substitution), and captures stdout to `iter-NNNN/summary.md` and into the record. Both `summary-prompt.md`/`summary.md` are written by the harness **outside** the scored worktree (under `iter-NNNN/`, not `wt/`), so they never trip deny enforcement. **Best-effort:** any failure (model unavailable, timeout, nonzero exit) must NOT abort or change the iteration outcome — log a warning, leave `summary` empty, fall back to `notes` alone.

  **Surface it.** Render a separate `## Recent attempt summaries` list in the prompt (last N iterations with a non-empty `summary`, e.g. `- iter 6 (discarded): <summary>`) so multi-sentence text isn't crammed into a table cell. Surface `summary` in `autorize status`. **Docs:** src/llms.md (§11 `summary` field; document `[summarize]`), src/templates/program.md.tmpl (prior-attempt summaries are appended automatically).

  **Open questions:** (1) confirm `claude --model haiku --print {prompt_file}` is the right invocation for the installed CLI — keep overridable. (2) Whether to feed the summarizer guidance + prior summaries (richer) vs diff+stdio only (cheaper) — default cheaper.

  **Acceptance:** (a) with `summarize.enabled = true` and a reachable (test: stub) summarize command, a non-`Noop` iteration gets a non-empty `summary` generated by a step that runs *after* the worker exits, bounded by `summarize.timeout` independently of `iteration.budget`; (b) the summary appears in the next iteration's prompt (`## Recent attempt summaries`) and in `autorize status`; (c) a summarizer failure/timeout leaves the iteration outcome unchanged and `summary` empty, with a warning logged; (d) `summarize.enabled = false` fully disables the step (no behavior change vs today except A1's `notes`); (e) existing `iterations.jsonl` logs (without `summary`) still load via serde defaults; (f) docs reflect the field + config section. Depends on A1.
...
- **[correctness/low-medium] `deny_paths` enforcement is blind to gitignored paths** — Deny enforcement works by `git add -A` then `git diff --name-only <branch>` (src/iteration.rs `CaptureDiff`), both of which honor `.gitignore` and `core.excludesFile`. If a deny pattern matches a path the repo (or the developer's global `core.excludesFile`) ignores — e.g. a machine that globally ignores `.autorize`/`logs/` — the agent's writes there are invisible to the diff, so the iteration reports `noop` instead of `denied`. In the common case the tampering is discarded with the worktree (no branch pollution), but the tampered file *is* present on disk during scoring and could skew the score, and `deny_paths` silently fails to mean what the user thinks. This surfaced as an environment-specific e2e failure (`deny_path_violation_yields_denied_outcome` went `noop` on a dev box whose `~/.gitignore` lists `.autorize`); the test was made hermetic via `core.excludesFile=/dev/null`, but the underlying product gap remains. Consider scanning deny paths against the worktree's actual file mutations (e.g. `git status --porcelain --ignored`, or `git add -A --force` for the deny scan only) rather than the ignore-filtered diff. (src/iteration.rs:91-104, src/worktree.rs `stage_all_in`/`diff_paths_against`)
