...
- **[data-integrity/high] No concurrency guard against two `autorize run` on the same experiment** — No lock file / flock / PID check. Two concurrent runs race on `state.json` (last writer wins), append interleaved records with duplicate iter numbers to `iterations.jsonl`, and both drive the tracking branch forward. Add a `flock`-based lock file under `.autorize/<name>/`. (src/cli/run.rs)
- **[data-integrity/medium] Mid-merge crash leaves branch advanced but record missing** — If killed between `commit_all_in` and `append_iteration` + `write_state`, the merge commit lives on `autorize/<name>` but state still says `iter_in_progress = N` with no `best_iter` update. Resume records the iter as `killed` / `score: null` even though it merged. Branch tip and log permanently disagree. Need a consistency check on resume that reconciles branch tip vs. log. (src/iteration.rs:143-189, src/cli/run.rs:209-238)
